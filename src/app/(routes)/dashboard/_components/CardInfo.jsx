...
function CardInfo({ budgetList, incomeList }) {
    ...
    const [totalSpend, setTotalSpent] = React.useState(0);
    const [totalIncome, setTotalIncome] = React.useState(0);
    const [financialAdvice, setFinancialAdvice] = React.useState("");
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        if (totalBudget > 0 || totalSpend > 0 || totaIncome > 0) {
            const fetchFinancialAdvice = async () => {
                const advice = await getFinancialAdvice(totalBudget, totalSpend, totaIncome);
